...
This section synthesizes research gaps and relates them to the companion research plan.

\paragraph{Companion-plan focus (decode-centric refinement).}
While the taxonomy above motivates \emph{integrated} L1--L4 co-design, the companion research plan deliberately \emph{narrows} the doctoral knife-edge to long-context \emph{decode}: real cache-path low-bit or mixed-precision KV representation, streaming fused dequantization that avoids materializing a full high-precision KV tile, and precision-aware mapping under ASIC cost models, validated through architecture simulation and critical-path RTL/PPA.
Full-stack ISA/compiler completeness, RMSNorm/RoPE as primary contributions, and sparse/PIM/chiplet paths remain out of the mainline; PLENA-class stacks serve as comparators rather than templates to enlarge~\cite{plena2025,bitdecoding2026,sawint42026}.
...
The gaps above still motivate closing L1--L4 concerns \emph{on one platform}, but the companion plan prioritizes a decode-centric subset rather than a broader PLENA-like full stack.
Differentiation vectors for that refined agenda:

\paragraph{Streaming low-bit KV datapath (L2-first, L1-supporting)}
Realize packed or paged compressed-KV fetch with fused dequantization into $QK^\top$/online softmax/$PV$, without default materialization of a full FP16 KV tile~\cite{bitdecoding2026,sawint42026,liu2024kivi}.
FlashAttention-native reductions remain necessary plumbing, not the sole claimed novelty~\cite{lin2025systolicattention,plena2025}.

\paragraph{Precision as a first-class mapping variable (L2+L4)}
Treat bit width, group or RoPE-block containers, tile shapes, and SRAM/DMA allocation as a joint search under accuracy and buffer caps, counting metadata and control overhead in traffic and energy~\cite{plena2025,batquant2026,sawint42026}.

\paragraph{Calibrated evaluation chain}
Retain three-layer calibration---algorithmic golden models, attention-aware architecture simulation, and RTL PPA for critical blocks---while reporting prefill only as a secondary appendix~\cite{parashar2019timeloop,scalesimv3_2025,klhufek2025transinfersim,lin2025systolicattention}.

\paragraph{Quantifiable targets}
% TODO: calibrate against FlashAttention/FlashDecoding and PLENA baselines after R1 modeling
Relative to GPU FlashAttention/FlashDecoding and documented monolithic references, goals emphasize reduced HBM bytes per decode token and energy per token at 32K--128K context under a Pareto accuracy envelope comparable to SAW-INT4/KIVI-class schemes~\cite{sawint42026,liu2024kivi,dao2023flashdecoding}.
Absolute multipliers remain \emph{to be calibrated} once baselines are locked (Table~\ref{tab:metrics}).

Adjacent sparse and memory-centric systems (Section~\ref{sec:adjacent}) remain upper-bound comparators and scope guards~\cite{amma2026,salca2026,plena2025}.
...
Section~\ref{sec:gaps} argued that integrated co-design remains necessary, while the companion research plan refines the doctoral focus to decode-centric streaming low-bit or mixed-precision KV datapaths with precision-aware mapping and calibrated architecture/RTL evaluation---rather than enlarging a full L1--L4 ISA/compiler stack in isolation.
Until end-to-end artifact chains become common for that knife-edge, reported energy-per-token and utilization gains will remain difficult to interpret across the L1--L4 literature.
