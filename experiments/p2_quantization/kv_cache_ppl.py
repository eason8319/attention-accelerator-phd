"""KV cache INT4 quantization perplexity on WikiText-2 (or offline synthetic corpus)."""
...
class _QuantizedLinearWrapper(nn.Module):
    """Wrap a linear layer with fake-quant on its output (simulates quantized KV)."""
...
def _make_quant_fn(mode: Mode, head_dim: int, seed: int):
    ...
    def fn(t: torch.Tensor) -> torch.Tensor:
        x = t
        if rot is not None:
            # Projections are (batch, seq, hidden); rotate last dim in head slices after reshape
            b, s, h = x.shape
            n_heads = h // head_dim
            ...
            x = x.reshape(b, s, h)
        q = int4_fake_quant(
            x, granularity=QuantGranularity.PER_GROUP, group_size=32, axis=-1
        )
        if rot is not None:
            b, s, h = q.shape
            n_heads = h // head_dim
...
def _patch_kv_projections(model: nn.Module, mode: Mode, seed: int = 0) -> list[tuple[nn.Module, nn.Module]]:
    """Wrap k_proj and v_proj with quant wrappers; return handles for restore."""
    head_dim = model.config.head_dim if getattr(model.config, "head_dim", None) else (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="KV cache INT4 PPL evaluation")
