...
> **状态**：已锁定（2026-07-24；2026-07-27 修订路径表述；2026-09-04 记录阶段 B 已用于 M3）。后续实验与报告必须引用本文件；若修订，须改版本号并在 CHANGELOG 记一笔。  
> **版本**：v1.2  
...
执行状态（2026-09-04）：**M3 已用阶段 B** 跑完协议锚模型（Llama-2-7B Table 3、Mistral-7B LongBench）。M4 可在阶段 A 开发；M5 主 Pareto 与 $D(16384,1024)$ 必须阶段 B。7B/8B 权重仅阶段 B 使用，勿把阶段 A 短序列写成正式结果。
