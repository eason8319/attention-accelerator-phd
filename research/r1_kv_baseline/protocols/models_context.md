...
> **状态**：已锁定（2026-07-24；2026-07-27 修订路径表述；2026-09-04 记录阶段 B 已用于KIVI 评估）。后续实验与报告必须引用本文件；若修订，须改版本号并在 CHANGELOG 记一笔。  
> **版本**：v1.2  
...
资源分工按是否实际加载模型区分：7B/8B 整模精度使用阶段 B；合成布局、模型几何记账和 CPU cache 字节采集可以在阶段 A 执行。后者不得声称测得整模生成质量或物理 HBM 性能。实际完成范围见 [R1 报告](../REPORT.md)，当前进度只在[进度页](../../../docs/progress/milestones.md)维护。
