...
**统计**：$n{=}20$ 独立随机种子（$0..19$），报告 **mean ± 样本标准差**  
...
| 随机种子 | $\{0,\ldots,19\}$（$n{=}20$），报告 **mean ± 样本标准差** |
...
$S{=}256$，$n{=}20$（mean±样本标准差）：

| 格式 | cosine | rel-$\ell_2$ | max-abs | total bytes |
|------|--------|--------------|---------|-------------|
| fp16 | $1.000000\pm 8\times 10^{-7}$ | $(2.077\pm 0.003)\times 10^{-4}$ | $(1.86\pm 0.09)\times 10^{-3}$ | 524288 |
| int8 | $0.999985\pm 6\times 10^{-7}$ | $(5.935\pm 0.015)\times 10^{-3}$ | $(1.82\pm 0.06)\times 10^{-2}$ | 270336 |
| int4 | $0.995331\pm 2\times 10^{-5}$ | $(9.702\pm 0.018)\times 10^{-2}$ | $(3.27\pm 0.12)\times 10^{-1}$ | 147456 |

相对 FP16 存储：INT8 总字节约 **0.52×**，INT4 约 **0.28×**（含 scale 元数据）。  
跨种子标准差相对均值很小（INT4 rel-$\ell_2$ 的 $\mathrm{std}/\mathrm{mean}\approx 0.2\%$），分层结论对随机初始化不敏感。  
$S\in\{64,256,1024\}$ 时各格式 rel-$\ell_2$ **几乎不随长度变化**，重建误差由量化网格主导。
...
| 格式 | cosine | rel-$\ell_2$ | max-abs | total bytes |
|------|--------|--------------|---------|-------------|
| fp16 | $1.000001\pm 5\times 10^{-7}$ | $(3.04\pm 0.05)\times 10^{-4}$ | $(3.5\pm 0.9)\times 10^{-4}$ | 524288 |
| int8 | $0.999965\pm 1\times 10^{-6}$ | $(8.49\pm 0.12)\times 10^{-3}$ | $(7.0\pm 1.1)\times 10^{-3}$ | 270336 |
| int4 | $0.99048\pm 2\times 10^{-4}$ | $(1.391\pm 0.017)\times 10^{-1}$ | $(1.12\pm 0.16)\times 10^{-1}$ | 147456 |

相对 FP16 attention 输出误差量级（按均值）：

- INT8 ≈ **28×** FP16 的 rel-$\ell_2$  
- INT4 ≈ **457×** FP16，约 **16×** INT8  

图（本地）`results/prefill_rel_l2_bars.png`：柱高为均值，误差棒为样本标准差；三档误差随 $S$ 几乎平坦。

对比 §3.1：同一格式下，**attention 输出 rel-$\ell_2$ 略高于 KV 重建**（INT4：$0.097\to 0.139$），Softmax 路径轻度放大，无数量级爆炸。
...
| 格式 | cosine | rel-$\ell_2$ | max-abs | total bytes |
|------|--------|--------------|---------|-------------|
| fp16 | $1.000000\pm 1\times 10^{-7}$ | $(3.02\pm 0.23)\times 10^{-4}$ | $(1.17\pm 0.23)\times 10^{-4}$ | 524288 |
| int8 | $0.999966\pm 4\times 10^{-6}$ | $(8.29\pm 0.51)\times 10^{-3}$ | $(2.82\pm 0.56)\times 10^{-3}$ | 270336 |
| int4 | $0.9910\pm 1\times 10^{-3}$ | $(1.36\pm 0.08)\times 10^{-1}$ | $(4.67\pm 0.77)\times 10^{-2}$ | 147456 |

与同长度 prefill（$S{=}256$）同量级。图（本地）`results/decode_error_vs_step.png` 仅展示 rel-$\ell_2$（mean±std）：

- 前 $\sim$16–64 step 内略升后 **平台化**；跨种子波动远小于格式间差距；  
- **未观察到**随 decode 步数近似线性发散的误差雪崩（$t\le 256$）；  
- cosine 仍见表中数值（FP16 $\approx 1$，INT8 $>0.99996$，INT4 $\approx 0.991$），不再单独作 $1-\mathrm{cosine}$ 图（近零量级不便对数展示）。
...
| int4 | $\approx 0.991$ | $\sim 0.14$ | 0.28 | $\approx 72\%$ |

INT4 metadata 在 $S{=}256$ 约占总量 $11\%$；结构随 token 线性。

---

## 4. 结论

1. **编码对真实 cache-path 精度有清晰分层**（$n{=}20$ 下 std 远小于格式间差距）：FP16 接近机器噪声；对称 INT8 保持 cosine $>0.99996$、rel-$\ell_2\sim 10^{-2}$；均匀 INT4 cosine $\approx 0.991$，rel-$\ell_2\sim 0.14$。  
...
### 5.2 关键数值摘录（decode，$t{=}256$，mean±std，$n{=}20$）

见本地 `results/summary_mean_std.csv`；正文表格与之一致。
