...
# 本环境是 R1 / 步骤 2 探测用 torch 2.5.1+cu121；不要在此安装 CUDA 扩展、BitDecoding 或 SAW。

source /cluster/apps/miniconda3/etc/profile.d/conda.sh
conda activate r1-kv-baseline

# 模型缓存约定见仓库根 AGENTS.md「模型权重缓存」；已导出的 HF_HOME 不被覆盖。
if [ -z "${HF_HOME:-}" ]; then
  export HF_HOME="${HOME}/hf-cache"
fi
export PIP_CACHE_DIR="${HOME}/.cache/pip"
mkdir -p "$HF_HOME/hub" "$PIP_CACHE_DIR"
...
echo "[r1-kv-baseline] HF_HOME=$HF_HOME | HF_ENDPOINT=$HF_ENDPOINT | inventory=$HF_HOME/INVENTORY.json"
