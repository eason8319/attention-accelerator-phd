...
## 2026-09-16（R2 步骤 6 能耗方法定向补录）

- 定向核验 Accelergy（ICCAD 2019，DOI `10.1109/ICCAD45719.2019.8942149`），复用既有键 `wu2019accelergy`；登记 CACTI 插件提交 `9cf48b12` 的 HBM2 $3.9\,\mathrm{pJ/bit}$ 常数与 ISPASS 2020 教程 45 nm 16-bit MAC $5\,\mathrm{pJ}$ 示例。写入台账 adjacent 与手册 §3.18 卡片、§5 对照行，不进入 §2 主对照表。不是全领域查新或历史性能复核。

## 2026-09-15（文献汇报图表定向核对）

- 定向复核既有 KIVI、SAW-INT4、BitDecoding、PLENA 四项来源的机制与所选结果，同步手册卡片/总览和台账的证据位置、负载与版本哈希；PLENA 本地稿系统表为 Table XII，旧总览 Table VIII 及峰值数字的版本对应关系未确认，改用 90K/8K 的明确比较行。未新增文献或引用键，未改原始 PDF；本次为汇报所需增量核验，不是全领域查新、全表数字复核或项目复现。

## 2026-09-14（手册结构）

- 删除对比手册顶部修订记录，后续只在本文件登记变更。
- 删除手册 §3.16「R2 评测来源总览」表；RULER、LongBench 卡片改为 §3.16、§3.17，锚点 `#ruler`、`#longbench` 不变。

## 2026-09-14（R2 公开内核依赖核对）

- 在既有 BitDecoding、SAW-INT4 条目补入固定作者提交、SGLang 子模块及构建/后端要求；FlashAttention 作者 README 作为 SAW 的运行依赖来源关联登记。手册卡片与台账同步，复用 `bitdecoding2026`、`sawint42026` 引用键，不新增平行来源。仅核对运行前提，未安装或测量公开内核，不重报论文性能或声称全面文献复核。

## 2026-09-14（R2 评测协议来源）

- 补录 RULER（COLM 2024）与原版 LongBench（ACL 2024），同步手册总览/卡片、台账和既有 BibTeX；固定作者代码提交，核对任务、生成预算、评分与版本边界。既有 R2 七项来源的引用键和证据角色覆盖已检查，本次不复核全部历史性能数字，也不代表全领域查新或本项目复现。
