# 研究进展

> 本目录记录博士课题进展；**阶段定义以 [`../research_plan.md`](../research_plan.md) 的 R0–R5 为准**。

## 文件说明

| 文件 | 用途 |
|------|------|
| [README.md](README.md) | 本文件 |
| [milestones.md](milestones.md) | R0–R5 checklist |
| [CHANGELOG.md](CHANGELOG.md) | 按日期进展摘要 |
| [logs/](logs/) | 可选详细日志 |

## 更新习惯

1. **开工前**：`git pull`  
2. **收工后**：更新 `CHANGELOG.md`（及必要时 `milestones.md`），再 commit + push  

## 当前阶段

- **R0**：已完成（survey、learning P1–P5、计划收敛、对比手册 / lit_watch）  
- **R1**：进行中（M0–M1 完成）— 见 [`../../research/r1_kv_baseline/`](../../research/r1_kv_baseline/)
- 文献增量更新：遵循 `docs/lit_watch/` + academic-researcher skill（`.cursor/rules/lit-watch-academic-researcher.mdc`）
