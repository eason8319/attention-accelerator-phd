...
StreamAttention pursues the same goal through a four-stage streaming operand schedule and piecewise-linear $\exp$, targeting higher sustained utilization than prior single-array FlashAttention engines that reportedly stall at $\sim$40\% PE occupancy.
...
StreamAttention adopts piecewise-linear $\exp$ inside a streaming single-array schedule~\cite{streamattention2025}.
...
Block-sparse GPU kernels such as block-sparse FlashAttention~\cite{dao2022flashattention_sparse} provide algorithmic references but assume programmer-specified sparsity patterns rather than hardware-driven pruning.
...
Single-array ASICs (SystolicAttention, StreamAttention, COSA) optimize softmax mapping but largely assume FP16 internal precision and omit systematic KV quantization studies~\cite{lin2025systolicattention,streamattention2025,wang2023cosa}.
