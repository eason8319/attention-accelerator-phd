#!/usr/bin/env python3
"""Cross-validate Roofline, SCALE-Sim, and Timeloop results for P3."""
...
# Assumed clock for converting SCALE-Sim cycles into attained TOPS.
# Absolute TOPS is illustrative; relative prefill/decode trends matter more.
ASSUMED_FREQ_HZ = 1e9
PEAK_TOPS = 128.0
RIDGE_AI = 128.0
# 32×32 MAC array at the assumed clock (2 ops per MAC).
...
def build_joined_table(
    ...
    roof = {
        (r["mode"], as_int(r, "seq_len"), r["gemm"]): r for r in roofline
    }
    time = {
        (r["mode"], as_int(r, "seq_len"), r["gemm"]): r for r in timeloop
    }
    ...
    for s in scalesim:
        ...
        r = roof[key]
        t = time[key]
        # Prefer tile×repetitions so head multiplicity is included.
        ops = gemm_ops(
            as_int(s, "tile_M"), as_int(s, "tile_N"), as_int(s, "tile_K")
        ) * as_int(s, "tile_repetitions")
        cycles = as_float(s, "total_cycles")
        attained_tops = (
            ops / cycles * ASSUMED_FREQ_HZ / 1e12 if cycles > 0 else 0.0
        )
...
def plot_traffic_energy(
    ...
    # Left: WS DRAM vs SRAM traffic for QK_T+PV layer slice
    ...
    # Right: Timeloop energy stack for decode vs prefill at each seq
    components = (
    ...
    for mode, seq in order:
        rows = [
            r
            for r in timeloop
            if r["mode"] == mode and as_int(r, "seq_len") == seq
        ]
        for key, _, name in components:
            series[name].append(
                sum(as_float(r, key) for r in rows) / 1e9
            )
...
def plot_roofline(
    ...
    array_roof = np.minimum(
        ARRAY_PEAK_TOPS, ai_line * (PEAK_TOPS / RIDGE_AI)
    )
    ...
    # One point per GEMM from WS SCALE-Sim + Roofline AI
    ...
    for row in joined:
        if row["dataflow"] != "ws":
            continue
        # Plot one representative seq (4K) to avoid clutter; size encodes seq
        if int(row["seq_len"]) not in SEQ_LENS:
            continue
        mode = str(row["mode"])
        gemm = str(row["gemm"])
        key = (mode, gemm)
        # Plot all seqs with alpha/size variation
...
def write_cross_summary(
    ...
    # Decode vs prefill util extremes for the narrative
    ws_qk = [
        r
        for r in scalesim
        if r["dataflow"] == "ws" and r["gemm"] == "QK_T"
    ]
    p_util = as_float(
            ...
            for gemm in ("QK_T", "PV"):
                ...
                if not d < p:
                    raise AssertionError(
                        f"{dataflow} {gemm} seq={seq}: decode util "
                        f"{d} not below prefill {p}"
                    )
    decode_bounds = {
        str(r["roofline_bound"])
        for r in joined
        if r["mode"] == "decode" and r["dataflow"] == "ws"
    }
    if decode_bounds != {"memory"}:
        raise AssertionError(
            f"expected all decode GEMMs memory-bound, got {decode_bounds}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OUTPUTS,
        help="directory containing tool CSVs and receiving plots",
    )
...
def main() -> None:
    ...
    print(
        "Validation passed: decode util < prefill and decode is memory-bound."
    )
