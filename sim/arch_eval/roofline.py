#!/usr/bin/env python3
"""Analytical Roofline model for one LLaMA-7B-scale attention layer.

The model deliberately uses a simple, reproducible traffic assumption: each
GEMM reads A and B once from HBM and writes C once. It is a theoretical
baseline, not a cache- or tiling-aware simulator.
"""
...
def attention_gemms(
    mode: str, seq_len: int, model: ModelConfig
) -> tuple[Gemm, ...]:
    """Return the GEMM sequence for one attention layer.

    Per-head QK^T/PV GEMMs are represented as one batched GEMM by folding
    batch and head count into M. This preserves aggregate operation and
    traffic counts while exposing the decode M=number-of-heads skinny shape.
    """
    if seq_len <= 0:
    ...
    attention_rows = model.batch * model.heads * (
        seq_len if mode == "prefill" else 1
    )
...
def evaluate_gemm(
    ...
    """Evaluate one GEMM using ops=2MNK and bytes=(MK+KN+MN)*element_bytes."""
    ops = 2 * gemm.m * gemm.n * gemm.k
    bytes_moved = (
        gemm.m * gemm.k + gemm.k * gemm.n + gemm.m * gemm.n
    ) * hardware.element_bytes
    ai = ops / bytes_moved
...
def write_markdown(
    ...
    lines = [
        ...
        "## Assumption",
        "",
        "Each GEMM reads A/B once from HBM and writes C once; "
        "ops count uses $2MNK$.",
        "",
        "## Hardware",
        "",
    ...
    lookup = {
        (row.mode, row.seq_len, row.gemm.name): row
        for row in rows
    }
...
def validate_expected_trend(rows: Sequence[RooflineRow]) -> None:
    """Fail loudly if the central P3 Roofline conclusion is not reproduced."""
    lookup = {
        (row.mode, row.seq_len, row.gemm.name): row
        for row in rows
    }
        ...
        for name in ("QK_T", "PV"):
            ...
            if decode.bound != "memory":
                raise AssertionError(
                    f"expected decode {name} at seq={seq_len} to be memory-bound"
                )
            if decode.ai >= prefill.ai:
                raise AssertionError(
                    f"expected decode {name} AI to be below prefill AI"
                )
...
def main() -> None:
    ...
    print(
        "Sanity check passed: decode QK_T/PV are memory-bound "
        "and have lower AI than prefill."
    )
