...
**证据来源**：[pytest.log](results/numerics_check/pytest.log)、[junit.xml](results/numerics_check/junit.xml)；源码与结果清单见 [experiment.json](experiment.json)。
...
本次在服务器 CPU 环境执行既有 `test_numerics.py`，使用 PyTorch 2.5.1+cu121 的 CPU 路径。在本实验目录的复现命令为 `python -m pytest test_numerics.py -q -p no:cacheprovider --junitxml=results/numerics_check/junit.xml`；原始日志与逐项检查状态完整保存。
