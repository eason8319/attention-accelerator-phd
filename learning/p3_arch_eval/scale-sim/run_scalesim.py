#!/usr/bin/env python3
"""对 attention GEMM 运行有界 tile 的 SCALE-Sim 扫描。

SCALE-Sim 会显式物化 operand 与 demand 矩阵，因此整块 32K/128K prefill GEMM 不现实。
本 runner 对每个精确 tile 形状仿真一次代表实例，并将周期与访问次数乘以精确 tile 数。
跨 tile 复用与 overlap 有意不建模。
"""
...
def write_layout_stub() -> Path:
    # SCALE-Sim 3.0.0 即使关闭 custom layout 也要求提供 layout 路径。
...
def collect(
    ...
    for item, comp, access in zip(workloads, compute, detail, strict=True):
        ...
        row: dict[str, object] = {
            ...
            "total_cycles": (as_int(comp, "Total Cycles (incl. prefetch)") * repeats),
            ...
            "mapping_efficiency_pct": as_float(comp, "Mapping Efficiency %"),
        ...
        for level, operand, action in (
            ...
            report_operand = operand.upper() if operand in {"ifmap", "ofmap"} else operand.title()
            report_key = f"{level.upper()} {report_operand} {action}"
            row[f"{level}_{operand}_{action.lower()}"] = as_int(access, report_key) * repeats
...
def validate(rows: list[dict[str, object]]) -> None:
    ...
    for dataflow in DATAFLOWS:
        for seq_len in SEQ_LENS:
            for gemm in ("QK_T", "PV"):
                prefill = float(lookup[(dataflow, "prefill", seq_len, gemm)]["overall_util_pct"])
                decode = float(lookup[(dataflow, "decode", seq_len, gemm)]["overall_util_pct"])
