#!/usr/bin/env python3
"""在与 SCALE-Sim 相同有界 tile 上运行 Timeloop/Accelergy。"""
...
def load_workloads() -> list[dict[str, str]]:
    ...
    # WS 与 OS 行的 GEMM/tile 元数据相同；只保留一份。
    selected = [row for row in rows if row["dataflow"] == "ws"]
    if len(selected) != 24:
        raise RuntimeError(f"expected 24 SCALE-Sim workloads, found {len(selected)}")
    return selected
...
def write_problem(directory: Path, m: int, n: int, k: int) -> None:
    ...
    # GEMM M×K @ K×N 表示为 1×1 卷积：
    # P=M, C=K, K=N；其余维度均为 1。
...
def run_tiles(
    ...
    for m, n, k in sorted({shape_key(row) for row in workloads}):
        ...
        results[(m, n, k)] = parse_stats(directory / "timeloop-mapper.stats.txt")
    return results
...
def build_energy_rows(
    ...
    for workload in workloads:
        ...
        components = {
            "mac_energy_pj": energy_pj(tile.mac_fj_per_compute, tile.computes, repeats),
            "register_energy_pj": energy_pj(tile.register_fj_per_compute, tile.computes, repeats),
            "sram_energy_pj": energy_pj(tile.sram_fj_per_compute, tile.computes, repeats),
            "dram_energy_pj": energy_pj(tile.dram_fj_per_compute, tile.computes, repeats),
        }
        ...
        rows.append(
            {
                ...
                **components,
                ...
                "register_pct": (100 * components["register_energy_pj"] / total),
