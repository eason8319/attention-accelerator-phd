...
# P1 attention 参考实现（将父目录加入 import 路径）
...
def _extract_qkv(
    model, input_ids: torch.Tensor, layer_idx: int = 0
    ...
    head_dim = (
        cfg.head_dim if hasattr(cfg, "head_dim") and cfg.head_dim else cfg.hidden_size // n_heads
    )
...
def analyze_activations(
    ...
    with torch.no_grad():
        ...
        # 离线 tiny 模型缺少自然 outlier；为演示放大若干通道。
        # 真实 checkpoint（如 Qwen）已有通道 outlier — 保持原样。
    ...
    def quant_k_direct(t):
        return int4_fake_quant(t, granularity=QuantGranularity.PER_GROUP, group_size=32, axis=-1)
    ...
    axes[0, 1].hist(
        k[0, 0].float().numpy().ravel(), bins=60, alpha=0.6, label="original", density=True
    )
    ...
    configs = [
        ...
        (
            "K Had+INT4",
            lambda qq, kk, vv: (qq, _quant_with_optional_rotation(kk, quant_k_direct, had_rot), vv),
            8,
        ),
        (
            "QK INT4 + V FP8",
    ...
    for b, e, lb in zip(bits_list, err_list, labels, strict=True):
    ...
    results["mixed_precision"] = list(zip(labels, bits_list, err_list, strict=True))
    return results
...
def main() -> None:
    parser = argparse.ArgumentParser(description="P2 量化误差分析")
