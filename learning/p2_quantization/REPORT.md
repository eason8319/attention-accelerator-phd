...
```bash
# 单元测试
conda run -n p2-quantization pytest test_fakequant.py -q --tb=short

# 误差分析（自动出图 + Markdown 报告）
conda run -n p2-quantization python error_analysis.py \
  --model Qwen/Qwen2.5-0.5B-Instruct

# KV cache INT4 困惑度对比
conda run -n p2-quantization python kv_cache_ppl.py \
  --model Qwen/Qwen2.5-0.5B-Instruct
```
