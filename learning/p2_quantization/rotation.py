"""用于抑制 outlier 的 Hadamard 与块对角旋转。"""
...
def _hadamard_matrix(n: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    """返回归一化的 n×n Hadamard 矩阵（n 须为 2 的幂）。"""
    if n & (n - 1) != 0:
        raise ValueError(f"Hadamard 尺寸须为 2 的幂，得到 {n}")
    h = hadamard(n)
...
def random_hadamard_matrix(
    ...
    *,
    ...
    """随机符号 Hadamard：D @ H，其中 D 为随机 ±1 对角阵。"""
    if dim & (dim - 1) != 0:
        # 填充到下一个 2 的幂以构造 Hadamard；必要时由调用方处理 padding
        next_pow2 = 1 << (dim - 1).bit_length()
        raise ValueError(
            f"dim {dim} 须为 2 的幂以使用 Hadamard；请设 pad_to_pow2=True 或 dim={next_pow2}"
        )
...
class RandomHadamardRotation:
    """沿 head_dim 应用随机 Hadamard 旋转（填充到 2 的幂）。"""
...
class BlockDiagonalRotation:
    """块对角 Walsh–Hadamard 旋转（BDR），QuaRot/SAW-INT4 风格。

    构造 ``R = block_diag(H, ..., H) @ D``，其中 ``H`` 为尺寸 ``block_size`` 的
    归一化 Walsh–Hadamard 矩阵，``D`` 为全维 ``dim`` 上的随机 ±1 对角阵。

    将随机符号放在输入侧（右乘 ``D``）可使各量化组与平坦 Hadamard 块对齐，
    并避免逐块 ``D_b @ H`` 或 Gaussian-QR 正交块带来的严重 seed 敏感性。
    块内 ``|R_ij| = 1/sqrt(block_size)``。
    """
    ...
    def __init__(
        ...
        if dim % block_size != 0:
            raise ValueError(f"dim {dim} 须能被 block_size {block_size} 整除")
        if block_size & (block_size - 1) != 0:
            raise ValueError(f"Hadamard 的 block_size 须为 2 的幂，得到 {block_size}")
        self.dim = dim
